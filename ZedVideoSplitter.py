...
_, frame = cap.read()
(h, w) = frame.shape[:2]
left_frame_writer = cv2.VideoWriter('Left_Frame.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30, (int(w / 2), h))
right_frame_writer = cv2.VideoWriter('Right_Frame.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30, (int(w / 2), h))
while True:
    retr, frame = cap.read()
    if retr == True:
